package guco.sfe.freemarker.cache;

import com.raiyee.intergration.ssf.freemarker.config.Configuration;
 ...
 */
public class TemplateCacheEnhancedAfter {
    ...
    private static final Logger logger = LoggerFactory.getLogger(TemplateCacheEnhancedAfter.class);
    ...
    private static final Pattern FREEMARK_TAG_START_PATTERN = Pattern.compile("<#(.|\\s)*?>");
    private static final Pattern FREEMARK_TAG_END_PATTERN = Pattern.compile("</#(.|\\s)*?>");
    private static final Pattern FREEMARK_MACRO_START_PATTERN = Pattern.compile("<@(.|\\s)*?>");
    private static final Pattern FREEMARK_MACRO_END_PATTERN = Pattern.compile("</@(.|\\s)*?>");
    private static final ThreadLocal<Map<String, String>> UNDERSCORE_SCRIPT = new ThreadLocal<>();
    private static final ThreadLocal<Map<String, String>> FREEMARKER_SCRIPT = new ThreadLocal<>();
    /**
     * Maybe {@code null}.
     */
     ...
     */
    public TemplateCacheEnhancedAfter() {
     ...
     */
    public TemplateCacheEnhancedAfter(TemplateLoader templateLoader) {
     ...
     * @deprecated Use {@link #TemplateCache(TemplateLoader, CacheStorage, Configuration)} instead.
     */
    public TemplateCacheEnhancedAfter(TemplateLoader templateLoader, CacheStorage cacheStorage) {
     ...
     * @since 2.3.21
     */
    public TemplateCacheEnhancedAfter(TemplateLoader defaultTemplateLoader, Configuration config) {
     ...
     * @since 2.3.21
     */
    public TemplateCacheEnhancedAfter(TemplateLoader templateLoader, CacheStorage cacheStorage, Configuration config) {
    ...
    private Template getTemplate(TemplateLoader loader, String name, Locale locale, String encoding, boolean parse)
        ...
        try {
            ...
            TemplateLoader templateLoader = config.getRemoteTemplateLoader();
            if (templateLoader != null &&
                    ((RemoteTemplateLoader) templateLoader).getRemotePaths()
                            .stream().anyMatch(path -> name.matches(path))
                    ) {
                t = loadEnhanceTemplate(loader, name, locale, encoding, parse, source);
            } else {
                t = loadTemplate(loader, name, locale, encoding, parse, source);
            }
    ...
    private Template loadEnhanceTemplate(TemplateLoader loader, String name, Locale locale, String encoding,
                                         boolean parse, Object source) throws IOException {
        Template template;
        Reader reader = loader.getReader(source, encoding);
        try {
            String templateSource;
            try {
                templateSource = processTemplate(reader, encoding);
            } catch (IOException e) {
                logger.error("{} is not available", name, e);
                throw new RuntimeException(e);
            }
            if (parse) {

                try {
                    template = new Template(name, templateSource, config);
                } catch (Template.WrongEncodingException wee) {
                    encoding = wee.specifiedEncoding;
                    reader.close();
                    reader = loader.getReader(source, encoding);
                ...
                template = Template.getPlainTextTemplate(name, templateSource, config);
                template.setLocale(locale);
            }
            template.setEncoding(encoding);
        } finally {
    ...
    protected String processTemplate(Reader reader, String encoding) throws IOException {
        Document doc = Jsoup.parse(
                escapeUnderscoreScript(
                        escapeFreemarkerScript(
                                IOUtils.toString(new ReaderInputStream(reader, encoding), encoding),
                                matchEnd -> "#freemarker-script#" + matchEnd
                        ),
        ...
        return unescapeUnderscoreScript(unescapeFreemarkerScript(doc.html()));
    }
    ...
    private void saxCustomAttributes(Document doc) {
        ...
        allElements.forEach(element -> {
            ...
            if ("t".equals(element.tagName())) {
                element.remove();
            } else if ("script".equals(element.tagName())) {
                if (element.hasAttr("script-constant")) {
                    StringBuilder paramsBuilder = new StringBuilder();
                    element.attributes().forEach(attribute -> {
                        if (!"type".equals(attribute.getKey()) && !"script-constant".equals(attribute.getKey())) {
                            String[] nameType = attribute.getKey().split("-");
                            String name = nameType[0];
                            String type = nameType.length == 2 ? nameType[1] : "string";
                            paramsBuilder.append("var " + name + " = ");
                            if ("number".equalsIgnoreCase(type)) {
                                paramsBuilder.append(Long.parseLong(attribute.getValue()));
                            } else {
                                paramsBuilder.append("\"" + attribute.getValue() + "\"");
                            }
                            paramsBuilder.append(";");
                        }
                    });
                    element.replaceWith(new Element(Tag.valueOf("script"), "").text(paramsBuilder.toString()));
                } else if (element.hasAttr("script-ftl")) {
                    element.replaceWith(new TextNode(element.html(), null).text(element.html()));
                }
            }
        });
    }

    private String unescapeFreemarkerScript(String escapedTemplateSource) {
        Map<String, String> freemarkerScript = FREEMARKER_SCRIPT.get();
        if (freemarkerScript == null) return escapedTemplateSource;

        for (String scriptIndex : freemarkerScript.keySet()) {
            escapedTemplateSource = escapedTemplateSource.replace(scriptIndex, freemarkerScript.get(scriptIndex));
        }
    ...
    private String escapeFreemarkerScript(String templateSource, Function<String, String> mapper) throws IOException {
        Map<String, String> freemarkerIndex = new HashMap<>();
        Matcher freemarkTagStartMatcher = FREEMARK_TAG_START_PATTERN.matcher(templateSource);

        while (freemarkTagStartMatcher.find()) {
            String scriptIndex = mapper.apply(UUID.randomUUID().toString());//"#freemarker-script#" + freemarkTagMatcher.end();
            freemarkerIndex.put(scriptIndex, freemarkTagStartMatcher.group());
            templateSource = freemarkTagStartMatcher.replaceFirst(scriptIndex);
            freemarkTagStartMatcher.reset(templateSource);
        }
        Matcher freemarkTagEndMatcher = FREEMARK_TAG_END_PATTERN.matcher(templateSource);
        while (freemarkTagEndMatcher.find()) {
            String scriptIndex = mapper.apply(UUID.randomUUID().toString());//"#freemarker-script#" + freemarkTagMatcher.end();
            freemarkerIndex.put(scriptIndex, freemarkTagEndMatcher.group());
            templateSource = freemarkTagEndMatcher.replaceFirst(scriptIndex);
            freemarkTagEndMatcher.reset(templateSource);
        }
        Matcher freemarkMacroStartMatcher = FREEMARK_MACRO_START_PATTERN.matcher(templateSource);
        while (freemarkMacroStartMatcher.find()) {
            String scriptIndex = mapper.apply(UUID.randomUUID().toString());//"#freemarker-script#" + freemarkTagMatcher.end();
            freemarkerIndex.put(scriptIndex, freemarkMacroStartMatcher.group());
            templateSource = freemarkMacroStartMatcher.replaceFirst(scriptIndex);
            freemarkMacroStartMatcher.reset(templateSource);
        }
        Matcher freemarkMacroEndMatcher = FREEMARK_MACRO_END_PATTERN.matcher(templateSource);
        while (freemarkMacroEndMatcher.find()) {
            String scriptIndex = mapper.apply(UUID.randomUUID().toString());//"#freemarker-script#" + freemarkTagMatcher.end();
            freemarkerIndex.put(scriptIndex, freemarkMacroEndMatcher.group());
            templateSource = freemarkMacroEndMatcher.replaceFirst(scriptIndex);
            freemarkMacroEndMatcher.reset(templateSource);
        }
        FREEMARKER_SCRIPT.set(freemarkerIndex);
        return templateSource;
    }
