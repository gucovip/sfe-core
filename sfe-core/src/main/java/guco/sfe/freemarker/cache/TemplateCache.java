...
public class TemplateCache {
    ...
    private static final String TAG_SCRIPT_CONSTANT_REGEX = "(?i)<script.*?script-constant([^>]*)>(.|\\s)*?</script>";
    private static final String TAG_SCRIPT_FTL_REGEX = "(?i)<script.*?script-ftl.*?>((.|\\s)*?)</script>";
...
//            t = loadTemplate(loader, name, locale, encoding, parse, source);
    ...
    private Template loadEnhanceTemplate(TemplateLoader loader, String name, Locale locale, String encoding,
        ...
        try {
            ...
            try {
                templateSource = processTemplate(reader, encoding).replaceAll("</(\\s*|.*)?@shiro(\\..*?>)", "</@shiro$2");
            } catch (IOException e) {
    ...
    protected String processTemplate(Reader reader, String encoding) throws IOException {
        return IOUtils.toString(new ReaderInputStream(reader, encoding), encoding).replaceAll(TAG_SCRIPT_CONSTANT_REGEX, "<script>$1</script>").replaceAll(TAG_SCRIPT_FTL_REGEX, "$1");
    }
